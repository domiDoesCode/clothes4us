import React, { useEffect, useState, useContext } from "react";
import axios from "axios";
import "../components/Cards.css";
import Button from "../components/Button";
import "./Cart.css";
import { Link, Redirect } from "react-router-dom";
import { CartContext } from "../context/CartContext";
import { UserContext } from "../context/UserContext";
import { toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

export default function Cart() {
  const [totalPrice, setTotalPrice] = useState(0);
  const { cartProducts, setCartProducts, refreshCartProducts } =
    useContext(CartContext);
  const { user } = useContext(UserContext);
  const notify = (productName, message) => {
    if (message === "add") {
      toast.success("+1 " + productName, {
        position: "bottom-right",
        autoClose: 5000,
        hideProgressBar: true,
        closeOnClick: true,
        pauseOnHover: true,
        draggable: true,
        progress: undefined,
      });
    } else {
      toast.success("-1 " + productName, {
        position: "bottom-right",
        autoClose: 5000,
        hideProgressBar: true,
        closeOnClick: true,
        pauseOnHover: true,
        draggable: true,
        progress: undefined,
      });
    }
  };
  let userId;
  if (user != null) {
    userId = user.userId;
  }

  const setProductsArray = async () => {
    const res = await axios.get(
      `http://localhost:5000/api/User/${userId}/cart`
    );
    setCartProducts(res.data);
  };

  const deleteProductFromCart = async (product) => {
    await axios.delete(`http://localhost:5000/api/User/${userId}/cart/${product.productId}`, {
      params: {
        productId: product.productId,
      },
    });
    setProductsArray();
  };

  const onClickClearCart = async () => {
    const res = await axios.delete(
      `http://localhost:5000/api/User/${userId}/cart`
    );
    if (res.status === 200) {
      setCartProducts([]);
    }
  };

  const onClickAddQuantity = async (product) => {
    console.log(product);
    const res = await axios.post(
      `http://localhost:5000/api/User/${userId}/cart/add`,
      product
    );
    if (res.status === 200) {
      notify(product.name, "add");
      refreshCartProducts();
    }
  };

  const onClickRemoveQuantity = async (product) => {
    const res = await axios.post(
      `http://localhost:5000/api/User/${userId}/cart/remove`,
      product
    );
    if (res.status === 200) {
      notify(product.name, "remove");
      refreshCartProducts();
    }
  };

  useEffect(() => {
    setProductsArray();
  }, []);

  useEffect(() => {
    let sumTotal = 0;
    if (cartProducts.length > 0) {
      cartProducts.forEach((x) => {
        sumTotal += x.price * x.quantity;
      });
      setTotalPrice(sumTotal);
    }
  }, [cartProducts]);

  useEffect(() => {
    if (cartProducts.length > 0) {
      cartProducts.forEach((x) => {
        if (x.quantity == 0) {
          deleteProductFromCart(x);
        }
      });
    }
  }, [cartProducts]);

  return (
    <>
      <div className="cards">
        {user == null ? <Redirect to="login"></Redirect> : <></>}
        {cartProducts.length > 0 ? (
          <div className="cart_header">
            <h1>Content of your cart:</h1>
            <h1>Total price: {totalPrice}</h1>
          </div>
        ) : (
          <h1>Your cart is empty</h1>
        )}
        {cartProducts.length > 0 ? (
          <>
            {cartProducts.map((x) => (
              <div className="horizontal_cards_container">
                <div className="pic">
                  <img src="data:image/jpeg;base64," />
                </div>
                <div className="desc">
                  <h3>{x.name}</h3>
                  <h5>Type: {x.type}</h5>
                  <h5>Price: {x.price}dkk</h5>
                </div>
                <i
                  className="fas fa-plus"
                  onClick={() => onClickAddQuantity(x)}
                ></i>

                <p>{x.quantity}</p>
                <i
                  className="fas fa-minus"
                  onClick={() => onClickRemoveQuantity(x)}
                ></i>
                <div className="horizontal_cards_thrash_icon">
                  <i
                    className="fas fa-trash-alt"
                    onClick={() => deleteProductFromCart(x)}
                  ></i>
                </div>
              </div>
            ))}{" "}
          </>
        ) : (
          <></>
        )}
        <div className="cart_buttons_clear_checkout">
          {cartProducts.length > 0 ? (
            <Button
              value="Clear cart"
              className="cart_button_clear"
              onClick={onClickClearCart}
            />
          ) : (
            <></>
          )}
          {cartProducts.length > 0 ? (
            <Link to="/shipping">
              <Button value="To shipping" className="cart_button_checkout" />
            </Link>
          ) : (
            <></>
          )}
        </div>
      </div>
    </>
  );
}
